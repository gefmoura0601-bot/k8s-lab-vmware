...
topology_deployment() {
  ...
  [[ "${available:-0}" == "${desired:-0}" ]] && ok "$title" "Deployment/$name ${available}/${desired} disponível" || warning "$title" "Deployment/$name ${available:-0}/${desired:-0} disponível"
}
...
topology_statefulset() {
  ...
  [[ "${ready:-0}" == "${desired:-0}" ]] && ok "$title" "StatefulSet/$name ${ready}/${desired} Ready" || warning "$title" "StatefulSet/$name ${ready:-0}/${desired:-0} Ready"
}

topology_namespace() {
  local namespace="$1"
  kubectl get namespace "$namespace" >/dev/null 2>&1 && ok "Namespace" "$namespace presente" || warning "Namespace" "$namespace ausente"
}
...
topology_argocd() {
  ...
  [[ "$unhealthy" == 0 ]] && ok "Argo CD" "$count Application(s) Synced/Healthy" || warning "Argo CD" "$unhealthy de $count Application(s) fora de Synced/Healthy"
}
...
topology_calico() {
  ...
  [[ "$available" == True ]] && ok "Calico" "Available" || critical "Calico" "Tigerastatus Available=${available:-vazio}"
}
...
topology_istio() {
  ...
  if kubectl get peerauthentication -A >/dev/null 2>&1; then
    ...
    (( mtls > 0 )) && ok "Istio mTLS" "$mtls PeerAuthentication STRICT" || warning "Istio mTLS" "nenhuma PeerAuthentication STRICT"
...
topology_observability() {
  ...
  if kubectl get servicemonitor -A >/dev/null 2>&1; then
    ...
    (( targets > 0 )) && ok "Prometheus discovery" "$targets ServiceMonitor(s)" || warning "Prometheus discovery" "nenhum ServiceMonitor"
...
topology_data_and_messaging() {
  ...
  [[ "$pending" == 0 ]] && ok "Persistent volumes" "$pvcs PVC(s) Bound" || warning "Persistent volumes" "$pending PVC(s) não Bound"
}
...
topology_autoscaling_and_policies() {
  ...
  if kubectl get scaledobject -A >/dev/null 2>&1; then
    ...
    (( scaled == ready )) && ok "KEDA" "$scaled ScaledObject(s) Ready" || warning "KEDA" "$ready/$scaled ScaledObject(s) Ready"
  ...
  if kubectl get clusterpolicy >/dev/null 2>&1; then
    ...
    (( policies > 0 )) && ok "Kyverno" "$policies ClusterPolicy(s) presente(s)" || warning "Kyverno" "nenhuma ClusterPolicy"
...
topology_workload_security() {
  ...
  (( policies > 0 )) && ok "NetworkPolicy" "$policies policy object(s)" || warning "NetworkPolicy" "nenhuma policy"
  ...
  (( probes > 0 )) && ok "Probes" "$probes container(s) com liveness/readiness" || warning "Probes" "nenhum container com ambas as probes"
}
